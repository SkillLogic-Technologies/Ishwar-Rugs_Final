...
export default function Login({ onOtpSent }: LoginProps = {}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        sessionStorage.setItem("otpUser", JSON.stringify({ email }));
  ...
  return (
    <div className="text-white mx-auto   p-8 rounded-lg bg-black/50 backdrop-blur-sm">
      ...
      {message && (
        <div
          className={`mb-4 text-center text-sm ${success ? "text-green-400" : "text-red-400"}`}
        >
          {message}
        </div>
      )}
